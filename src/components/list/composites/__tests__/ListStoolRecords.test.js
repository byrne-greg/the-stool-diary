...
describe("ListStoolRecords", () => {
  describe("UI", () => {
    ...
    describe("titleComponent", () => {
      test("when mounted with a title component, then it should display title component", () => {
        ...
        const { queryByTestId, queryByText } = render(
          <ListStoolRecords titleComponent={<h1>{mockTitle}</h1>} />
        )
    ...
    describe("displayDaySeparators", () => {
        ...
        const { queryByTestId } = render(
          <ListStoolRecords
            ...
            recordedStools={[
              {
                type: 1,
                dateTime: { timestamp: moment().format() },
                size: STOOL_SIZES.SMALL,
              },
            ]}
          />
        )
        const daySeparatorComponent = queryByTestId(
          "list-stool-records-day-separator"
        )
          ...
          <ListStoolRecords
            ...
            recordedStools={[
              ...
              },
              {
                type: 3,
                dateTime: { timestamp: moment().format() },
                size: STOOL_SIZES.LARGE,
              },
            ]}
          />
        )
        const day = queryByTestId("list-stool-records-day")
        const daySeparatorComponent = day.querySelectorAll(
          "[data-testid='list-stool-records-day-separator']"
        )
        const listStoolItems = day.querySelectorAll(
          "[data-testid='list-stool-item']"
        )
      ...
      test("when mounted with a multiple stool records on the different days, then it should display multiple stool cards under different day sections", () => {
        ...
        const { queryAllByTestId } = render(
          <ListStoolRecords
            ...
            recordedStools={[
              {
                type: 1,
                dateTime: { timestamp: moment().subtract(2, "days").format() },
                size: STOOL_SIZES.SMALL,
              },
              {
                type: 2,
                dateTime: { timestamp: moment().subtract(1, "days").format() },
                size: STOOL_SIZES.MEDIUM,
              },
              {
                type: 3,
        ...
        const daySeparatorComponent = queryAllByTestId(
          "list-stool-records-day-separator"
        )
    ...
    describe("hasSort", () => {
      test("when sorting is enabled, then it should display a sort button", () => {
        ...
        const sortButtonComponent = queryByTestId(
          "list-stool-records-sort-button"
        )
      ...
      test("when sorting is enabled and sortAscending is false, then the sort button should show latest to earliest", () => {
        ...
        // ASSERT
        expect(sortButtonComponent).toStrictEqual(
          `${defaultLocale["Sorted by"]} ${defaultLocale["Latest to Earliest"]}`
        )
      })
      test("when sorting is enabled and sortAscending is true, then the sort button should show earliest to latest", () => {
        ...
        // ASSERT
        expect(sortButtonComponent).toStrictEqual(
          `${defaultLocale["Sorted by"]} ${defaultLocale["Earliest to Latest"]}`
        )
      })
  ...
  describe("Functional", () => {
    test("when mounted, then stool records should be sorted by latest to earliest", () => {
      ...
      const { queryAllByTestId } = render(
        <ListStoolRecords
          recordedStools={[earliestRecord, medianRecord, latestRecord]}
        />
      )
      ...
      // ASSERT
      expect(listStoolItemTypes[0].textContent).toStrictEqual(
        `Type ${latestRecord.type}`
      )
      expect(listStoolItemTypes[1].textContent).toStrictEqual(
        `Type ${medianRecord.type}`
      )
      expect(listStoolItemTypes[2].textContent).toStrictEqual(
        `Type ${earliestRecord.type}`
      )
    })
