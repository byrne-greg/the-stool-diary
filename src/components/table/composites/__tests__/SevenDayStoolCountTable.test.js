...
const getDisplayDateFormatForMoment = moment =>
  `${moment.format("dddd")}, ${moment.format("Do")} ${moment.format("MMMM")}`
...
describe("SevenDayStoolCountTable", () => {
  describe("UI", () => {
    ...
    test("when mounted with a title, then it should render with the title", async () => {
      ...
      const { queryByText } = render(
        <SevenDayStoolCountTable title={<h1>{titleText}</h1>} />
      )
    ...
    test("when displayed, then the header columns should be a collapsible cell, Day and Stool Count", async () => {
      ...
      const collapsibleHeaderCell = queryByTestId(
        "collapsible-table-header-cell-collapsetoggle"
      )
      ...
      const isContainingDayInSecondHeaderColumn = screen.queryByText(
        defaultLocale["Day"]
      )

      // check the third column (second 'data' column)
      document.body.innerHTML = dataHeaderCells[1].outerHTML
      const isContainingStoolCountInThirdHeaderColumn = screen.queryByText(
        defaultLocale["Stool Count"]
      )
    ...
    test("when displayed with some stool data, then some rows should a non-zero stool count", async () => {
      ...
      const { getAllByTestId } = render(
        <SevenDayStoolCountTable recordedStools={stoolData} />
      )
      const stoolCounts = getAllByTestId("stool-count")
      ...
      const stoolData = [
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(7, "day").format(),
            dateString: moment()
              .subtract(7, "day")
              .format(momentFormatter.YYYYMMDD),
            dateOnly: false,
        ...
        },
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(8, "day").format(),
            dateString: moment()
              .subtract(8, "day")
              .format(momentFormatter.YYYYMMDD),
            dateOnly: false,
        ...
        },
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(9, "day").format(),
            dateString: moment()
              .subtract(9, "day")
              .format(momentFormatter.YYYYMMDD),
            dateOnly: false,
        ...
        },
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(10, "day").format(),
            dateString: moment()
              .subtract(10, "day")
              .format(momentFormatter.YYYYMMDD),
            dateOnly: false,
  ...
  describe("Functional", () => {
    test("when a row has zero stool counts, then the row does not expand", async () => {
      ...
      const collapsedRowCells = queryAllByTestId(
        "collapsible-table-body-collapsedrow-cell"
      )
    ...
    test("when a row has some stool counts, then the row can expand", async () => {
      ...
      const collapsedFirstRowCell = getAllByTestId(
        "collapsible-table-body-collapsedrow-cell"
      )[0]
    ...
    test("when the stool count column header is clicked, then it changes the stool row order to ascending by stool count", async () => {
      ...
      const stoolData = [
        ...
        },
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(4, "day").format(),
            dateString: moment()
              .subtract(4, "day")
              .format(momentFormatter.YYYYMMDD),
            dateOnly: false,
