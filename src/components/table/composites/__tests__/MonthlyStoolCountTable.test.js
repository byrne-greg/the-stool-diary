...
const getDisplayMonthFormatForMoment = moment =>
...
describe("MonthlyDayStoolCountTable", () => {
  describe("UI", () => {
    ...
    test("when mounted with a month, then it should render with the month on display", async () => {
      ...
      const { queryByTestId } = render(
        <MonthlyStoolCountTable month={mockMonth} />
      )
      const displayMonth = queryByTestId(
        "monthly-stool-count-table-displaymonth"
      ).textContent

      // ASSERT
      expect(displayMonth).toBe(
        getDisplayMonthFormatForMoment(moment(mockMonth))
      )
    })
    ...
    test("when mounted with a semantic title element, then it should render with the semantic title element", async () => {
      ...
      // ACT
      const { queryByTestId } = render(
        <MonthlyStoolCountTable semanticTitleElement={semanticTitle} />
      )
      const displayMonthElementTagName = queryByTestId(
        "monthly-stool-count-table-displaymonth"
      ).tagName
    ...
    test("when displayed, the previous month selector is not shown", async () => {
      ...
      // ACT
      const { queryByTestId } = render(<MonthlyStoolCountTable />)
      const previousMonthSelector = queryByTestId(
        "monthly-stool-count-table-previousmonthselector"
      )
    ...
    test("when displayed in the current month, the next month selector is not shown", async () => {
      ...
      // ACT
      const { queryByTestId } = render(<MonthlyStoolCountTable />)
      const nextMonthSelector = queryByTestId(
        "monthly-stool-count-table-nextmonthselector"
      )
    ...
    test("when displayed in a previous month, the next month selector is shown", async () => {
      ...
      // ACT
      const { queryByTestId } = render(
        <MonthlyStoolCountTable month={previousMonth} />
      )
      const nextMonthSelector = queryByTestId(
        "monthly-stool-count-table-nextmonthselector"
      )
    ...
    test("when displayed with no data, then all cells to current date will show zero stool counts", async () => {
      ...
      // ASSERT
      expect(
        actualStoolCounts.filter(stoolCount => stoolCount != 0)
      ).toStrictEqual([])
    })
    ...
    test("when displayed with particular stool data, then those cells will show a non-zero stool count", async () => {
      ...
      const stoolData = [
        {
          ...
          dateTime: {
            ...
            dateString: moment().format(momentFormatter.YYYYMMDD),
            dateOnly: false,
          },
        },
        {
      ...
      // ACT
      const { getAllByTestId } = render(
        <MonthlyStoolCountTable recordedStools={stoolData} />
      )
      const stoolCountCells = getAllByTestId("stool-count")
      stoolCountCells.forEach(stoolCountCell => {
        actualStoolCounts.push(stoolCountCell.textContent)
      ...
      expect(
        actualStoolCounts.filter(stoolCount => stoolCount != 0)
      ).toStrictEqual([stoolData.length + ""])
    })
  })
  ...
  describe("Functional", () => {
    test("when the previous month is selected, the previous month stool records are displayed", async () => {
      ...
      const stoolData = [
        {
          ...INITIAL_STATE,
          dateTime: {
            timestamp: moment().subtract(1, "month").format(),
            dateString: moment()
              .subtract(1, "month")
      ...
      fireEvent.click(
        getByTestId("monthly-stool-count-table-previousmonthselector")
      )
    ...
    test("when the previous month is selected, the previous month is shown", async () => {
      // ARRANGE
      const previousMonthDisplayName = getDisplayMonthFormatForMoment(
      ...
      const monthOnDisplay = getByTestId(
        "monthly-stool-count-table-displaymonth"
      ).textContent
    ...
    test("when the next month is selected, the next month stool records are displayed", async () => {
      ...
      const { getAllByTestId, getByTestId } = render(
        <MonthlyStoolCountTable
          month={previousMonth}
          recordedStools={stoolData}
        />
      )
      fireEvent.click(
        getByTestId("monthly-stool-count-table-nextmonthselector")
      )
