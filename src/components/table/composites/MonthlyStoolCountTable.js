...
const MonthlyStoolCountTable = ({
  month = moment().format("YYYYMM"),
  recordedStools = [],
  semanticTitleElement = "h4",
  displayTitleElement = null,
}) => {
    ...
    <div>
      <div className={classes.monthSelector}>
        ...
        <IconButton
          ...
          onClick={() => {
            setDisplayMonth(moment(displayMonth).subtract(1, "months"))
          }}
        ...
        <Typography
          variant={
            displayTitleElement ? displayTitleElement : semanticTitleElement
          }
          component={semanticTitleElement}
          data-testid="monthly-stool-count-table-displaymonth"
        >
          {`${t(moment(displayMonth).format("MMMM"))} - ${moment(
            displayMonth
          ).format("YYYY")}`}
        </Typography>
        ...
        {isBeforeCurrentDate(moment(displayMonth).add(1, "months")) ? (
          <IconButton
            ...
            onClick={() => {
              setDisplayMonth(
                moment(displayMonth).add(1, "months").format("YYYYMM")
              )
            }}
      ...
      <div className={classes.table}>
        <BaseStoolDayCountTable
          ...
          endDate={moment(displayMonth)
            .add(daysToAddSinceFirstDay, "days")
...
const isBeforeCurrentDate = dateString =>
  moment(dateString).isSameOrBefore(moment())
...
function getStoolTableData(stoolDayData, t) {
  ...
  const monthWeekNums = [
    ...new Set(stoolDayData.map(dayData => moment(dayData.dateString).week())),
  ]
  const stoolTableRows = monthWeekNums.map(weekNum => {
    const dayDataRecordsForWeek = stoolDayData.filter(
      dayData => moment(dayData.dateString).week() === weekNum
    )
    ...
    if (dayDataRecordsForWeek.length < 7) {
      ...
      tableRow.data.push(
        ...[...new Array(7).fill(null)].map((_, i) => {
          const dayDataForCell = dayDataRecordsForWeek.find(
            dayData => moment(dayData.dateString).day() === i
          )
          return dayDataForCell &&
            isBeforeCurrentDate(dayDataForCell.dateString)
            ? {
                display: (
                  <StoolCount count={dayDataForCell.count}>
                    {dayDataForCell.count}
                  </StoolCount>
                ),
      ...
      // full week
      tableRow.data.push(
        ...dayDataRecordsForWeek.map(dayData => {
    ...
    const daysWithRecords = tableRow.data.filter(
      dayData => dayData.stools && dayData.stools.length > 0
    )
    if (daysWithRecords.length > 0) {
      const mappedDaysWithRecords = daysWithRecords.map(
        dayData => dayData.stools
      )
      // .flat() is not supported in node 10 and older browsers
      const weekStoolRecords = mappedDaysWithRecords.reduce(
            ...
            <ListStoolRecords
              ...
              titleComponent={
                <Typography variant="h5">{`${t(
                  "Recorded stools during week"
                )} ${tableRow.data[0].value}`}</Typography>
              }
            />
          ),
        }
