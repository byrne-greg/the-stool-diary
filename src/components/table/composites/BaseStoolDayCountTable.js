...
const BaseStoolDayCountTable = ({
  ...
  const dayDataForDisplay = useMemo(() => {
    ...
    const dayDataForPeriod = [
      { dateString: endDateString, count: 0, stools: [] },
    ]
    const numberOfDaysBetweenStartAndEnd = moment(endDate).diff(
      moment(startDate),
      "days"
    )
    ...
    const sortedRecordedStools = recordedStools
      .slice()
      .sort(
        (a, b) =>
          new Date(b.dateTime.timestamp) - new Date(a.dateTime.timestamp)
      )
    ...
    sortedRecordedStools
      .filter(recordedStool => {
        return (
          moment(recordedStool.dateTime.dateString).isSameOrAfter(
            startDateString,
            "day"
          ) &&
          moment(recordedStool.dateTime.dateString).isSameOrBefore(
            endDateString,
            "day"
          )
        )
      ...
      .forEach(recordedStoolInDateRange => {
        const dayDataItem = dayDataForPeriod.find(dayData => {
          return (
            dayData.dateString === recordedStoolInDateRange.dateTime.dateString
          )
        })
        dayDataItem.count++
  ...
  const stoolTableData = useMemo(
    () => stoolDataTableDisplayFn(dayDataForDisplay, t),
    [dayDataForDisplay]
  )
      ...
      data: [
        {
          ...
          value: moment(dayData.dateString).format(),
          type: "date",
        },
        {
          display: (
