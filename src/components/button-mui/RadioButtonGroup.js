...
const RadioButtonGroup = ({
  ...
  const shouldUseVertical =
    orientation === "vertical" || useMediaQuery(theme.breakpoints.down("xs"))

  return (
    <MaterialButtonGroup
      orientation={shouldUseVertical ? "vertical" : "horizontal"}
...
const useRadioButtonStyles = makeStyles(theme => ({
  ...
  label: ({ isOrientationVertical, isChecked, buttonPalette }) => {
    ...
    return {
      ...
      color: isChecked
        ? theme.palette.getContrastText(buttonPalette.dark)
        : buttonPalette.main,
      borderLeft: isChecked
        ? getBorder(buttonPalette.dark)
        : getBorder(buttonPalette.main),
      borderTop: isChecked
        ? getBorder(buttonPalette.dark)
        : getBorder(buttonPalette.main),
      borderBottom: isOrientationVertical
        ? 0
        : isChecked
        ? getBorder(buttonPalette.dark)
        : getBorder(buttonPalette.main),
