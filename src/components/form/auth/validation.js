...
export const validateTextField = ({
  ...
  if (value === undefined)
    return { isInvalid: true, reason: "Something unexpected has occurred" }
  ...
  if (type === VALIDATION_TYPE.EMAIL) {
    ...
    if (value.match(/[^\w@\.]/g))
      return {
        isInvalid: true,
        reason: "Must not contain spaces or non-email characters",
      }

    // must contain one @ symbol
    const atSymMatches = [...value.matchAll(/\@/g)]
    if (atSymMatches.length != 1)
      return { isInvalid: true, reason: "Must contain one @ symbol" }
    ...
    const alphaCharMatchesBetweenStartAndAtSymSubstring = [
      ...betweenStartAndAtSymSubstring.matchAll(/[A-Za-z]/g),
    ]
    if (!alphaCharMatchesBetweenStartAndAtSymSubstring.length > 0)
      return {
        isInvalid: true,
        reason: "Must contain text characters before the @ symbol",
      }
    ...
    if (periodAfterAtSymIndex === -1)
      return {
        isInvalid: true,
        reason: "Must contain at least one period after @ symbol",
      }
    ...
    if (value.length < 8)
      return {
        ...
        reason:
          ...
          commonPasswordValidationReason,
      }
    ...
    if (!numberChars.length > 0)
      return {
        isInvalid: true,
        reason:
          "Password does not have a numeric character. " +
          commonPasswordValidationReason,
      }
  } else {
    // must contain more than 3 chars
    if (value.length < 3)
      return { isInvalid: true, reason: "Must contain more than 3 characters" }
