...
The Stool Diary uses `jest` and `react-testing-library` for unit testing the React components and agnostic/utility functions.
...
The Stool Diary uses `testcafe` for end-to-end testing (i.e. user simulation testing). Testcafe is configured for the project using `.testcaferc.json` file.
...
The Stool Diary uses `react-i18next` to provide text translations. This involves wrapping text with a hook provided by the i18n library.
